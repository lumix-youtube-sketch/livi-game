  ...
  const StatPill = ({ icon: Icon, value, color }) => (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
      <div className="glass-panel" style={{ width: '42px', height: '110px', borderRadius: '22px', background: 'rgba(255,255,255,0.02)', padding: '4px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', border: '1px solid rgba(255,255,255,0.08)', position: 'relative', overflow: 'hidden' }}>
        <motion.div initial={false} animate={{ height: `${Math.max(10, value)}%` }} transition={{ type: 'spring', stiffness: 50, damping: 20 }} style={{ width: '100%', borderRadius: '18px', background: `linear-gradient(to top, ${color}, ${color}88)`, boxShadow: `0 0 15px ${color}33` }} />
        <div style={{ position: 'absolute', top: '8px', left: 0, right: 0, textAlign: 'center' }}><Icon size={16} color="#fff" style={{ opacity: 0.8 }} /></div>
      </div>
  ...
  return (
    <div style={{ position: 'relative', height: '100vh', width: '100%', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '75px', left: '20px', right: '20px', display: 'flex', justifyContent: 'space-between', zIndex: 100 }}>
        <div className="hud-capsule"><Trophy size={16} color="#FFC312" /><span style={{ fontSize: '16px', fontWeight: 900 }}>LVL {pet.level}</span></div>
